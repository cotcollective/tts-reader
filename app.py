...
def api_load():
    ...
    # warning edge → remonte au frontend (visible dans l'UI, pas juste dans le log serveur)
    edge_chunks = result.get("edge_chunks", 0)
    warning = None
    if result.get("engine") == "edge" and edge_chunks > 0:
        warning = f"⚠️ {edge_chunks} chunk(s) envoyés à Microsoft (backend edge, voix {voice}). Texte non privé."
    return jsonify({"title": result["title"], "meta": result["meta"], "total_chunks": len(STATE["chunks_audio"]), "bookmark": bookmark, "start_chunk": start_chunk, "voice": voice, "engine": result.get("engine"), "warning": warning})
